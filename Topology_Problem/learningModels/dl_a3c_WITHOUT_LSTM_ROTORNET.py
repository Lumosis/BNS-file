...
class AC_Network():
    def __init__(self, nt_size, tm_size, a_size, scope, trainer):
        with tf.variable_scope(scope):
            ...
            self.policy = tf.reshape(self.policy,
                                     [-1, STEPS, a_size])  # [?, 10, 28]
            self.value = tf.reshape(self.value,
                                    [-1, STEPS, FLAGS.k_acts])  # [?, 10, 6]

            #Only the worker network need ops for loss functions and gradient updating.
            if scope != 'global':
                self.actions = tf.placeholder(
                    shape=[None, STEPS, FLAGS.k_acts],
                    dtype=tf.int32)  # [?, k_acts] -> [?, 10, 6]
                self.actions_onehot = tf.one_hot(
                    self.actions, a_size,
                    dtype=tf.float32)  # [?, k_acts, a_size] -> [?, 10, 6, 28]
                self.target_v = tf.placeholder(
                    shape=[None, STEPS, FLAGS.k_acts],
                    dtype=tf.float32)  # [?, 10, 6] (Discounted Rewards)

                # Positive reward
                self.advantages = tf.placeholder(
                    shape=[None, STEPS, FLAGS.k_acts], dtype=tf.float32
                )  # [?, 10, 6] (Generalized Advantage Estimation)
                self.policy_expand = tf.expand_dims(self.policy,
                                                    2)  # [?, 10, 1, 28]

                self.policy_mul = tf.tile(
                    self.policy_expand,
                    [1, 1, FLAGS.k_acts, 1])  # [?, 10, 6, 28]
                self.responsible_outputs = tf.reduce_sum(
                    self.policy_mul * self.actions_onehot, [3])  # [?, 10, 6]
        ...
        with sess.as_default(), sess.graph.as_default():
            while not coord.should_stop():
                ...
                while (self.env.recv() == False):
                    ...
                    temp_actions = np.zeros((STEPS, FLAGS.a_size))
                    temp_selects = []
                    ...
                    a_dist = a_dist[0]
                    a_dist = a_dist.reshape((STEPS, A_SIZE))

                    optical_links = []
                    for idx in xrange(STEPS):
                        temp_selects = [_ for _ in xrange(len(a_dist[idx]))]
                        prob_distribution = a_dist[idx] / a_dist[idx].sum()
                        temp_selects = np.random.choice(temp_selects,
                                                        size=FLAGS.k_acts,
                                                        replace=False,
                                                        p=prob_distribution)
                        temp_selects = temp_selects.tolist()
                        optical_links.append(temp_selects)

                        for a_sel in temp_selects:
                            temp_actions[idx][a_sel] = 1
                    ...
                    self.env.send(('OPTICAL_LINKS', optical_links))
                    s1, r, d = self.env.recv()

                    self.episode_rewards.append(np.sum(r))
                    self.episode_mean_values.append(np.mean(v[0]))

                    episode_buffer_a.append([s, optical_links, r, s1, d, v])
